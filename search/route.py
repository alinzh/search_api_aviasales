...
class Route:
    ...
    def append(self, x, tranzit=60) -> bool:
        ...
        if len(self.storage) >= 1:
            ...
            data_1 = datetime.strptime(
                first_data, date_format
            )  # дата вылета рейса storage[-1]
            flight_duration = dict_atrr_1[
                "time_in_sky"
            ]  # время полета рейса storage[-1], т.е. У
            tranzit = self.len_tranzit(x[0])
            if date_2 > data_1:
                delta = date_2 - (
                    data_1
                    + timedelta(minutes=flight_duration)
                    + timedelta(minutes=tranzit)
                )
